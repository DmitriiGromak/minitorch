...
def test_symmetric() -> None:
    ...
    assert_close(mul(-3.0, 7.0), mul(7.0, -3.0))
    assert_close(mul(0.0, 4.5), mul(4.5, 0.0))
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    """
    combined_sum = minitorch.sum(addLists(ls1, ls2))
    separate_sum = minitorch.sum(ls1) + minitorch.sum(ls2)
    assert_close(combined_sum, separate_sum)
